...
$nullVal = null;
$post = $_POST;
$sql = $post['table'] && $post['table'] === 'BART'
  ? 'INSERT INTO BARTDL ('.file_get_contents('bartdl.sql').') VALUES ('.implode(', ', array_fill(0, 23, '?')).')'
  : file_get_contents("recDef.sql");
...
if (!$post['table'] || $post['table'] !== 'BART') {
  ...
  echo "There was a problem committing your submission";
}
